...
class TransactionItemsDialogIn(QDialog):
    ...
    def _create_ui(self) -> QVBoxLayout:
        ...
        scroll_layout.addWidget(self.transaction_info_widget)
        scroll_layout.addWidget(self.transactions_items_widget, 3)
    ...
    def _get_item_data(
        ...
        if index is None or not index.isValid():
            return index, None
        data = self.transactions_items_widget.transaction_item_model_in.get_transaction_item_data(
            index
        )
        return index, data
