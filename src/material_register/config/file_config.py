from pathlib import Path

REQUIRED_CONFIG_FILES = [
    Path("config") / "settings.toml",
]

REQUIRED_JSON_FILES = [
    Path("cs_CZ") / "ui_texts.json",
    Path("en_GB") / "ui_texts.json",
    Path("cs_CZ") / "error_texts.json",
    Path("en_GB") / "error_texts.json",
    Path("cs_CZ") / "headers_texts.json",
    Path("en_GB") / "headers_texts.json",
    Path("cs_CZ") / "confirm_texts.json",
    Path("en_GB") / "confirm_texts.json",
    Path("cs_CZ") / "notification_texts.json",
    Path("en_GB") / "notification_texts.json",
    Path("cs_CZ") / "status_texts.json",
    Path("en_GB") / "status_texts.json",
    Path("cs_CZ") / "export_texts.json",
    Path("en_GB") / "export_texts.json",
]

REQUIRED_IMAGES = [
    Path("system") / "SplashScreen.jpg",
    Path("system") / "applicationIcon.png",
    Path("ui_icons") / "activeIcon.png",
    Path("ui_icons") / "inactiveIcon.png",
    Path("ui_icons") / "copyIcon.png",
    Path("ui_icons") / "deleteIcon.png",
    Path("tools_icons") / "notesButtonIcon.png",
    Path("tools_icons") / "cashBalanceButtonIcon.png",
    Path("tools_icons") / "databaseButtonIcon.png",
]

REQUIRED_STYLES_FILES = [
    Path("styles") / "dark_blue.qss",
]

UI_KEYS = [
    ("Application", "titleText"),
    ("MainWindow", "titleText"),
    ("SidePanel", "transactionsButtonText"),
    ("SidePanel", "transactionsButtonTooltipText"),
    ("SidePanel", "customersButtonText"),
    ("SidePanel", "customersButtonTooltipText"),
    ("SidePanel", "exportButtonText"),
    ("SidePanel", "exportButtonTooltipText"),
    ("SidePanel", "catalogButtonText"),
    ("SidePanel", "catalogButtonTooltipText"),
    ("SidePanel", "inventoryButtonText"),
    ("SidePanel", "inventoryButtonTooltipText"),
    ("SidePanel", "settingsButtonText"),
    ("SidePanel", "settingsButtonTooltipText"),
    ("TransactionsWidget", "modelInSuffix"),
    ("TransactionsWidget", "itemsCountLabelText"),
    ("TransactionsWidget", "pricePrefix"),
    ("TransactionsWidget", "priceSuffix"),
    ("TransactionsWidget", "tooltipCustomerText"),
    ("TransactionsWidget", "tooltipAddressText"),
    ("TransactionsWidget", "tooltipPaymentText"),
    ("TransactionsWidget", "CASH"),
    ("TransactionsWidget", "TRANSFER"),
    ("TransactionsWidget", "tooltipNotesText"),
    ("TransactionsView", "updateTransactionActionText"),
    ("TransactionsView", "deleteTransactionActionText"),
    ("TransactionsActionsWidget", "inTransactionButtonText"),
    ("TransactionsActionsWidget", "inTransactionButtonTooltipText"),
    ("TransactionsActionsWidget", "outTransactionButtonText"),
    ("TransactionsActionsWidget", "outTransactionButtonTooltipText"),
    ("TransactionsActionsWidget", "searchLineEditPlaceholderText"),
    ("TransactionsActionsWidget", "baseFilterComboboxItems"),
    ("TransactionsTabWidget", "inTabTitleText"),
    ("TransactionsTabWidget", "outTabTitleText"),
    ("SettingsDialog", "titleText"),
    ("SettingsDialog", "closeButtonText"),
    ("SettingsDialog", "closeButtonTooltipText"),
    ("SettingsSidePanel", "exportButtonText"),
    ("SettingsSidePanel", "exportButtonTooltipText"),
    ("SettingsSidePanel", "toolsButtonText"),
    ("SettingsSidePanel", "toolsButtonTooltipText"),
    ("CustomersActionsWidget", "addCustomerButtonText"),
    ("CustomersActionsWidget", "addCustomerButtonTooltipText"),
    ("CustomersActionsWidget", "searchLineEditPlaceholderText"),
    ("CustomersWidget", "countLabelText"),
    ("CustomersView", "updateCustomerActionText"),
    ("CustomersView", "activeCustomerActionText"),
    ("InventoryWidget", "countLabelText"),
    ("InventoryActionsWidget", "searchLineEditPlaceholderText"),
    ("ExportWidget", "summaryTabTitle"),
    ("ExportWidget", "transactionsTabTitle"),
    ("SummaryExportWidget", "branchGroupBoxText"),
    ("SummaryExportWidget", "pathNameGroupBoxText"),
    ("SummaryExportWidget", "fileTypeLabelText"),
    ("SummaryExportWidget", "fileTypeComboBoxItems"),
    ("SummaryExportWidget", "pathLabelText"),
    ("SummaryExportWidget", "pathButtonText"),
    ("SummaryExportWidget", "folderDialogTitle"),
    ("SummaryExportWidget", "fileNameLabelText"),
    ("SummaryExportWidget", "fileNameLineEditText"),
    ("SummaryExportWidget", "fileNameLineEditPlaceholderText"),
    ("SummaryExportWidget", "dateOptionsGroupBoxText"),
    ("SummaryExportWidget", "todayRadioButtonText"),
    ("SummaryExportWidget", "weekRadioButtonText"),
    ("SummaryExportWidget", "monthRadioButtonText"),
    ("SummaryExportWidget", "yearRadioButtonText"),
    ("SummaryExportWidget", "customRadioButtonText"),
    ("SummaryExportWidget", "fromDateLabelText"),
    ("SummaryExportWidget", "toDateLabelText"),
    ("SummaryExportWidget", "financialDataGroupBoxText"),
    ("SummaryExportWidget", "priceSuffix"),
    ("SummaryExportWidget", "openingBalanceLabelText"),
    ("SummaryExportWidget", "incomeLabelText"),
    ("SummaryExportWidget", "expenseLabelText"),
    ("SummaryExportWidget", "exportOptionsGroupBoxText"),
    ("SummaryExportWidget", "noActionRadioButtonText"),
    ("SummaryExportWidget", "openFolderRadioButtonText"),
    ("SummaryExportWidget", "openFileRadioButtonText"),
    ("SummaryExportWidget", "otherSettingsGroupBoxText"),
    ("SummaryExportWidget", "useLastOptionsCheckboxText"),
    ("SummaryExportWidget", "saveLastOpeningBalanceCheckboxText"),
    ("SummaryExportWidget", "exportButtonText"),
    ("SummaryExportWidget", "exportButtonTooltipText"),
    ("TransactionsExportWidget", "branchGroupBoxText"),
    ("TransactionsExportWidget", "branchNameLabelText"),
    ("TransactionsExportWidget", "branchNameLineEditPlaceholderText"),
    ("TransactionsExportWidget", "pathNameGroupBoxText"),
    ("TransactionsExportWidget", "fileTypeLabelText"),
    ("TransactionsExportWidget", "fileTypeComboBoxItems"),
    ("TransactionsExportWidget", "pathLabelText"),
    ("TransactionsExportWidget", "pathButtonText"),
    ("TransactionsExportWidget", "folderDialogTitle"),
    ("TransactionsExportWidget", "fileNameLabelText"),
    ("TransactionsExportWidget", "fileNameLineEditText"),
    ("TransactionsExportWidget", "fileNameLineEditPlaceholderText"),
    ("TransactionsExportWidget", "dateOptionsGroupBoxText"),
    ("TransactionsExportWidget", "todayRadioButtonText"),
    ("TransactionsExportWidget", "weekRadioButtonText"),
    ("TransactionsExportWidget", "monthRadioButtonText"),
    ("TransactionsExportWidget", "yearRadioButtonText"),
    ("TransactionsExportWidget", "customRadioButtonText"),
    ("TransactionsExportWidget", "fromDateLabelText"),
    ("TransactionsExportWidget", "toDateLabelText"),
    ("TransactionsExportWidget", "monthSplitCheckboxText"),
    ("TransactionsExportWidget", "transactionOptionsGroupBoxText"),
    ("TransactionsExportWidget", "customerLabelText"),
    ("TransactionsExportWidget", "allCustomersText"),
    ("TransactionsExportWidget", "transactionTypeLabelText"),
    ("TransactionsExportWidget", "allRadioButtonText"),
    ("TransactionsExportWidget", "inRadioButtonText"),
    ("TransactionsExportWidget", "outRadioButtonText"),
    ("TransactionsExportWidget", "exportOptionsGroupBoxText"),
    ("TransactionsExportWidget", "noActionRadioButtonText"),
    ("TransactionsExportWidget", "openFolderRadioButtonText"),
    ("TransactionsExportWidget", "openFileRadioButtonText"),
    ("TransactionsExportWidget", "otherSettingsGroupBoxText"),
    ("TransactionsExportWidget", "useLastOptionsCheckboxText"),
    ("TransactionsExportWidget", "exportButtonText"),
    ("TransactionsExportWidget", "exportButtonTooltipText"),
    ("ErrorDialog", "closeDialogButtonText"),
    ("ErrorDialog", "closeDialogButtonTooltipText"),
    ("ErrorDialog", "closeAppButtonText"),
    ("ErrorDialog", "closeAppButtonTooltipText"),
    ("CustomerDialog", "titleText"),
    ("CustomerDialog", "createdLabelText"),
    ("CustomerDialog", "subjectTypePlaceholderText"),
    ("CustomerDialog", "subjectTypeItems"),
    ("CustomerDialog", "companyLabelText"),
    ("CustomerDialog", "firstNameLabelText"),
    ("CustomerDialog", "lastNameLabelText"),
    ("CustomerDialog", "documentTypeLabelText"),
    ("CustomerDialog", "addressLabelText"),
    ("CustomerDialog", "activeLabelText"),
    ("CustomerDialog", "notesLabelText"),
    ("CustomerDialog", "notesCountLabelText"),
    ("CustomerDialog", "saveButtonText"),
    ("CustomerDialog", "saveButtonTooltipText"),
    ("CustomerDialog", "closeButtonText"),
    ("CustomerDialog", "closeButtonTooltipText"),
    ("CategoryDialog", "titleText"),
    ("CategoryDialog", "categoryNameLabelText"),
    ("CategoryDialog", "notesLabelText"),
    ("CategoryDialog", "notesCountLabelText"),
    ("CategoryDialog", "saveButtonText"),
    ("CategoryDialog", "saveButtonTooltipText"),
    ("CategoryDialog", "closeButtonText"),
    ("CategoryDialog", "closeButtonTooltipText"),
    ("CommodityDialog", "titleText"),
    ("CommodityDialog", "categoryLabelText"),
    ("CommodityDialog", "nameLabelText"),
    ("CommodityDialog", "unitLabelText"),
    ("CommodityDialog", "unitInputItems"),
    ("CommodityDialog", "warningLabelText"),
    ("CommodityDialog", "defaultPriceLabelText"),
    ("CommodityDialog", "activeLabelText"),
    ("CommodityDialog", "notesLabelText"),
    ("CommodityDialog", "notesCountLabelText"),
    ("CommodityDialog", "saveButtonText"),
    ("CommodityDialog", "saveButtonTooltipText"),
    ("CommodityDialog", "closeButtonText"),
    ("CommodityDialog", "closeButtonTooltipText"),
    ("CatalogWidget", "addCategoryButtonText"),
    ("CatalogWidget", "addCategoryButtonTooltipText"),
    ("CatalogWidget", "updateCommoditiesPriceText"),
    ("CatalogWidget", "updateCommoditiesPriceTooltipText"),
    ("CatalogWidget", "catalogTitleLabelText"),
    ("CatalogDefaultWidget", "titleLabelText"),
    ("CatalogDefaultWidget", "subtitleLabelText"),
    ("CategoryDetailWidget", "categoryGroupBoxText"),
    ("CategoryDetailWidget", "notesLabelText"),
    ("CategoryDetailWidget", "updateCategoryButtonText"),
    ("CategoryDetailWidget", "updateCategoryButtonTooltipText"),
    ("CategoryDetailWidget", "addCommodityButtonText"),
    ("CategoryDetailWidget", "addCommodityButtonTooltipText"),
    ("CommoditiesGridWidget", "commoditiesGroupBoxText"),
    ("CommoditiesGridWidget", "noCommoditiesLabelText"),
    ("CommodityCardWidget", "unitLabelText"),
    ("CommodityCardWidget", "defaultPriceLabelText"),
    ("CommodityCardWidget", "activeLabelText"),
    ("CommodityCardWidget", "notesLabelText"),
    ("CommodityCardWidget", "updateCommodityButtonText"),
    ("CommodityCardWidget", "updateCommodityButtonTooltipText"),
    ("UpdateCommoditiesPriceDialog", "titleText"),
    ("UpdateCommoditiesPriceDialog", "commodityNameLabelText"),
    ("UpdateCommoditiesPriceDialog", "defaultPriceLabelText"),
    ("UpdateCommoditiesPriceDialog", "newPriceLabelText"),
    ("UpdateCommoditiesPriceDialog", "saveButtonText"),
    ("UpdateCommoditiesPriceDialog", "saveButtonTooltipText"),
    ("UpdateCommoditiesPriceDialog", "cancelButtonText"),
    ("UpdateCommoditiesPriceDialog", "cancelButtonTooltipText"),
    ("CreateTransactionDialog", "titleText"),
    ("CreateTransactionDialog", "updateTitleText"),
    ("CreateTransactionDialog", "paymentTypeLabelText"),
    ("CreateTransactionDialog", "paymentTypeComboboxItems"),
    ("CreateTransactionDialog", "customerNameLabelText"),
    ("CreateTransactionDialog", "customerDocumentNumberLabelText"),
    ("CreateTransactionDialog", "customerAddressLabelText"),
    ("CreateTransactionDialog", "continueTransactionButtonText"),
    ("CreateTransactionDialog", "continueTransactionButtonTooltipText"),
    ("CreateTransactionDialog", "updateTransactionButtonText"),
    ("CreateTransactionDialog", "updateTransactionButtonTooltipText"),
    ("CreateTransactionDialog", "cancelTransactionButtonText"),
    ("CreateTransactionDialog", "cancelTransactionButtonTooltipText"),
    ("TransactionItemsDialogIn", "titleText"),
    ("TransactionItemsDialogIn", "saveTransactionButtonText"),
    ("TransactionItemsDialogIn", "saveTransactionButtonTooltipText"),
    ("TransactionItemsDialogIn", "cancelTransactionButtonText"),
    ("TransactionItemsDialogIn", "cancelTransactionButtonTooltipText"),
    ("TransactionItemsDialogIn", "CASH"),
    ("TransactionItemsDialogIn", "TRANSFER"),
    ("TransactionItemsDialogOut", "titleText"),
    ("TransactionItemsDialogOut", "saveTransactionButtonText"),
    ("TransactionItemsDialogOut", "saveTransactionButtonTooltipText"),
    ("TransactionItemsDialogOut", "cancelTransactionButtonText"),
    ("TransactionItemsDialogOut", "cancelTransactionButtonTooltipText"),
    ("TransactionItemsDialogOut", "CASH"),
    ("TransactionItemsDialogOut", "TRANSFER"),
    ("TransactionInfoWidget", "customerGroupBoxText"),
    ("TransactionInfoWidget", "customerNameLabelText"),
    ("TransactionInfoWidget", "documentNumberLabelText"),
    ("TransactionInfoWidget", "addressLabelText"),
    ("TransactionInfoWidget", "updateTransactionInfoButtonText"),
    ("TransactionInfoWidget", "updateTransactionInfoButtonTooltipText"),
    ("TransactionInfoWidget", "notesGroupBoxText"),
    ("TransactionInfoWidget", "notesCountLabelText"),
    ("TransactionsItemsWidget", "addItemButtonText"),
    ("TransactionsItemsWidget", "addItemButtonTooltipText"),
    ("TransactionsItemsWidget", "updateItemButtonText"),
    ("TransactionsItemsWidget", "updateItemButtonTooltipText"),
    ("TransactionsItemsWidget", "deleteButtonText"),
    ("TransactionsItemsWidget", "deleteButtonTooltipText"),
    ("TransactionsItemsWidget", "totalCountLabelText"),
    ("TransactionsItemsWidget", "totalPriceLabelText"),
    ("TransactionsItemsWidget", "priceSuffix"),
    ("CategoryCommodityDialog", "titleText"),
    ("CategoryCommodityDialog", "updateTitleText"),
    ("CategoryCommodityDialog", "categoryLabelText"),
    ("CategoryCommodityDialog", "categoryComboBoxPlaceholderText"),
    ("CategoryCommodityDialog", "commodityLabelText"),
    ("CategoryCommodityDialog", "commodityComboBoxPlaceholderText"),
    ("CategoryCommodityDialog", "unitLabelText"),
    ("CategoryCommodityDialog", "priceLabelText"),
    ("CategoryCommodityDialog", "addButtonText"),
    ("CategoryCommodityDialog", "addButtonTooltipText"),
    ("CategoryCommodityDialog", "updateButtonText"),
    ("CategoryCommodityDialog", "updateButtonTooltipText"),
    ("CategoryCommodityDialog", "cancelButtonText"),
    ("CategoryCommodityDialog", "cancelButtonTooltipText"),
    ("SettingsExportWidget", "summaryExportTabTitle"),
    ("SettingsExportWidget", "transactionsExportTabTitle"),
    ("BaseExportWidget", "branchGroupBoxText"),
    ("BaseExportWidget", "branchNameLabelText"),
    ("BaseExportWidget", "branchNameLineEditPlaceholderText"),
    ("BaseExportWidget", "pathNameGroupBoxText"),
    ("BaseExportWidget", "pathLabelText"),
    ("BaseExportWidget", "pathButtonText"),
    ("BaseExportWidget", "folderDialogTitle"),
    ("BaseExportWidget", "fileNameLabelText"),
    ("BaseExportWidget", "fileNameLineEditPlaceholderText"),
    ("BaseExportWidget", "exportOptionsGroupBoxText"),
    ("BaseExportWidget", "noActionRadioButtonText"),
    ("BaseExportWidget", "openFolderRadioButtonText"),
    ("BaseExportWidget", "openFileRadioButtonText"),
    ("BaseExportWidget", "otherSettingsGroupBoxText"),
    ("BaseExportWidget", "useLastOptionsCheckboxText"),
    ("BaseExportWidget", "saveLastOpeningBalanceCheckboxText"),
    ("BaseExportWidget", "settingsInfoLabelText"),
    ("BaseExportWidget", "restoreButtonText"),
    ("BaseExportWidget", "restoreButtonTooltipText"),
    ("BaseExportWidget", "saveButtonText"),
    ("BaseExportWidget", "saveButtonTooltipText"),
    ("SettingsToolsWidget", "toolsGroupBoxText"),
    ("SettingsToolsWidget", "containerSizeCheckboxText"),
    ("SettingsToolsWidget", "cashBalanceGroupBoxText"),
    ("SettingsToolsWidget", "balanceCashCheckboxText"),
    ("SettingsToolsWidget", "valuesCashCheckboxText"),
    ("SettingsToolsWidget", "othersCashCheckboxText"),
    ("SettingsToolsWidget", "settingsInfoLabelText"),
    ("SettingsToolsWidget", "restoreButtonText"),
    ("SettingsToolsWidget", "restoreButtonTooltipText"),
    ("SettingsToolsWidget", "saveButtonText"),
    ("SettingsToolsWidget", "saveButtonTooltipText"),
    ("RightToolbarWidget", "notesButtonTooltipText"),
    ("RightToolbarWidget", "cashBalanceButtonTooltipText"),
    ("RightToolbarWidget", "databaseButtonTooltipText"),
    ("NotesWidget", "permanentGroupBoxText"),
    ("NotesWidget", "permanentNotesEditPlaceholderText"),
    ("NotesWidget", "searchEditPlaceholderText"),
    ("NotesWidget", "permanentCopyButtonTooltipText"),
    ("NotesWidget", "permanentDeleteButtonTooltipText"),
    ("NotesWidget", "localGroupBoxText"),
    ("NotesWidget", "localNotesEditPlaceholderText"),
    ("NotesWidget", "localCopyButtonTooltipText"),
    ("NotesWidget", "localDeleteButtonTooltipText"),
    ("CashBalanceWidget", "cashTabText"),
    ("CashBalanceWidget", "openingBalanceLabelText"),
    ("CashBalanceWidget", "transactionCashLabelText"),
    ("CashBalanceWidget", "incomeLabelText"),
    ("CashBalanceWidget", "expenseLabelText"),
    ("CashBalanceWidget", "balanceCountLabelText"),
    ("CashBalanceWidget", "valuesTabText"),
    ("CashBalanceWidget", "valueItems"),
    ("CashBalanceWidget", "othersLabelText"),
    ("CashBalanceWidget", "cashTotalLabelText"),
    ("CashBalanceWidget", "totalLabelText"),
    ("CashBalanceWidget", "currencySuffix"),
    ("CashBalanceWidget", "quantitySuffix"),
    ("DatabaseBackupWidget", "infoGroupBoxText"),
    ("DatabaseBackupWidget", "databaseNameLabelText"),
    ("DatabaseBackupWidget", "databaseSizeLabelText"),
    ("DatabaseBackupWidget", "databaseLastModifiedLabelText"),
    ("DatabaseBackupWidget", "databaseLastBackupLabelText"),
    ("DatabaseBackupWidget", "customBackupButtonText"),
    ("DatabaseBackupWidget", "customBackupButtonTooltipText"),
    ("DatabaseBackupWidget", "folderBackupDialogTitle"),
    ("DatabaseBackupWidget", "noBackupLabelText"),
    ("DatabaseBackupWidget", "selectedBackupLabelText"),
    ("DatabaseBackupWidget", "selectedPathLabelText"),
    ("DatabaseBackupWidget", "customRestoreButtonText"),
    ("DatabaseBackupWidget", "customRestoreButtonTooltipText"),
    ("DatabaseBackupWidget", "fileRestoreDialogTitle"),
    ("DatabaseBackupWidget", "fileRestoreDialogFilterText"),
    ("DatabaseBackupWidget", "restoreBackupButtonText"),
    ("DatabaseBackupWidget", "restoreBackupButtonTooltipText"),
    ("ProgressDialog", "loadingDataText"),
    ("ProgressDialog", "exportInProgressText"),
    ("ProgressDialog", "restoreInProgressText"),
    ("ProgressDialog", "restartApplicationText"),
]

HEADERS_KEYS = [
    ("CustomersView", "company"),
    ("CustomersView", "document_number"),
    ("CustomersView", "address"),
    ("CustomersView", "active"),
    ("TransactionView", "category"),
    ("TransactionView", "commodity"),
    ("TransactionView", "commodityId"),
    ("TransactionView", "unitCount"),
    ("TransactionView", "pricePerUnit"),
    ("TransactionView", "totalPrice"),
    ("TransactionsView", "headers_texts"),
    ("InventoryView", "0"),
    ("InventoryView", "1"),
    ("InventoryView", "2"),
    ("InventoryView", "3"),
    ("InventoryView", "4"),
]

ERROR_KEYS = [
    "APP_INIT_FAILED",
    "RESOURCES_MISSING",
    "DOWNLOAD_FAILED",
    "TEXTS_LOAD_FAILED",
    "ICONS_LOAD_FAILED",
    "CONFIG_LOAD_FAILED",
    "SETTINGS_FAILED",
    "DATABASE_FAILED",
    "DATABASE_ERROR",
    "EXPORT_ERROR",
    "BACKUP_ERROR",
    "RESTORE_ERROR",
    "PATH_ERROR",
    "CRITICAL_FAILURE",
    "UNKNOWN_ERROR",
    "CONNECTION_ERROR",
    "PERMISSION_ERROR",
]

CONFIRM_STRUCTURE = {
    "ACTIVE": ["TITLE", "TEXT", "YES", "NO"],
    "NO_RESULTS": ["TITLE", "TEXT", "CLOSE"],
    "NO_CUSTOMERS": ["TITLE", "TEXT", "CLOSE"],
    "NO_CATEGORY": ["TITLE", "TEXT", "CLOSE"],
    "TREE_ITEM_CREATION_FAILED": ["TITLE", "TEXT", "CLOSE"],
    "NO_DATA": ["TITLE", "TEXT", "CLOSE"],
    "INVALID_DATA": ["TITLE", "TEXT", "CLOSE"],
    "ITEMS_DATA_FAILED": ["TITLE", "TEXT", "CLOSE"],
    "INVALID_COMMODITY_UNIT": ["TITLE", "TEXT", "CLOSE"],
    "ZERO_PRICE_TRANSACTION": ["TITLE", "TEXT", "YES", "NO"],
    "DELETE_TRANSACTION_ITEM": ["TITLE", "TEXT", "YES", "NO"],
    "DATABASE_ERROR": ["TITLE", "TEXT", "CLOSE"],
    "DELETE_TRANSACTION": ["TITLE", "TEXT", "YES", "NO"],
    "RESTORE_SETTINGS": ["TITLE", "TEXT", "YES", "NO"],
    "FILE_EXISTS": ["TITLE", "TEXT", "YES", "NO"],
    "FOLDER_EXISTS": ["TITLE", "TEXT", "YES", "NO"],
    "OPEN_FILE_FAILED": ["TITLE", "TEXT", "CLOSE"],
    "OPEN_EXPLORER_FAILED": ["TITLE", "TEXT", "CLOSE"],
    "CUSTOM_BACKUP": ["TITLE", "TEXT", "YES", "NO"],
    "RESTORE_DATABASE": ["TITLE", "TEXT", "YES", "NO"],
}

NOTIFICATION_KEYS = [
    ("STARTUP", "DATABASE_RESTORED"),
    ("CUSTOMERS", "ADD_CUSTOMER"),
    ("CUSTOMERS", "UPDATE_CUSTOMER"),
    ("CUSTOMERS", "CHANGE_ACTIVE"),
    ("CATALOG", "ADD_CATEGORY"),
    ("CATALOG", "UPDATE_CATEGORY"),
    ("CATALOG", "ADD_COMMODITY"),
    ("CATALOG", "UPDATE_COMMODITY"),
    ("CATALOG", "UPDATE_PRICE"),
    ("TRANSACTIONS", "ADD_TRANSACTION"),
    ("TRANSACTIONS", "UPDATE_TRANSACTION"),
    ("TRANSACTIONS", "DELETE_TRANSACTION"),
    ("EXPORT", "EXPORT_COMPLETED"),
    ("BACKUP", "BACKUP_CREATED"),
]

STATUS_KEYS = [
    "START_BACKUP",
    "BACKUP_FAILED",
    "BACKUP_CREATED",
    "LOAD_NOTES_FAILED",
    "SETTINGS_SAVED",
    "SETTINGS_RESTORED",
    "READY",
]

EXPORT_KEYS = [
    ("SummarySheet", "sheetName"),
    ("SummarySheet", "titleText"),
    ("SummarySheet", "rangeText"),
    ("SummarySheet", "branchText"),
    ("SummarySheet", "notesText"),
    ("SummarySheet", "financialText"),
    ("SummarySheet", "openingBalanceText"),
    ("SummarySheet", "buybackText"),
    ("SummarySheet", "cashText"),
    ("SummarySheet", "transferText"),
    ("SummarySheet", "incomeText"),
    ("SummarySheet", "expenseText"),
    ("SummarySheet", "balanceText"),
    ("SummarySheet", "currencySuffix"),
    ("SummarySheet", "commodityText"),
    ("SummarySheet", "paymentTypeText"),
    ("SummarySheet", "CASH"),
    ("SummarySheet", "TRANSFER"),
    ("SummarySheet", "pricePerUnitText"),
    ("SummarySheet", "quantityText"),
    ("SummarySheet", "totalPriceText"),
    ("SummarySheet", "summaryPriceText"),
    ("SummarySheet", "exportText"),
    ("SummarySheet", "pageText"),
    ("TransactionsSheet", "titleText"),
    ("TransactionsSheet", "rangeText"),
    ("TransactionsSheet", "branchText"),
    ("TransactionsSheet", "notesText"),
    ("TransactionsSheet", "financialText"),
    ("TransactionsSheet", "customerNameText"),
    ("TransactionsSheet", "documentNumberText"),
    ("TransactionsSheet", "addressText"),
    ("TransactionsSheet", "paymentTypeText"),
    ("TransactionsSheet", "CASH"),
    ("TransactionsSheet", "TRANSFER"),
    ("TransactionsSheet", "IN"),
    ("TransactionsSheet", "OUT"),
    ("TransactionsSheet", "createdAtText"),
    ("TransactionsSheet", "currencySuffix"),
    ("TransactionsSheet", "categoryText"),
    ("TransactionsSheet", "commodityText"),
    ("TransactionsSheet", "pricePerUnitText"),
    ("TransactionsSheet", "quantityText"),
    ("TransactionsSheet", "totalPriceText"),
    ("TransactionsSheet", "summaryPriceText"),
    ("TransactionsSheet", "countText"),
]
