...
class TransactionsService:
    ...
    def update_transaction(
        ...
        try:
            ...
            ok, error = TransactionItemsQueries.delete_transaction_items(
            ...
            for commodity_id, amount in final_stock_dict.items():
                ok, inventory_error = InventoryQueries.update_inventory_item(
                    db_connection, commodity_id, amount
                )
                if not ok:
                    db_connection.rollback()
    ...
    def delete_transaction(
        ...
        try:
            ...
            for item in items:
                ...
                ok, error = InventoryQueries.update_inventory_item(
                    db_connection, item.commodity_id, amount
                )
                if not ok:
                    db_connection.rollback()
                    return False, error
            ok, error = TransactionsQueries.delete_transaction(
                db_connection, transaction_id
            )
            if not ok:
                db_connection.rollback()
    ...
    @staticmethod
    def _get_amount(
        transfer_type: str, amount: float, negate: bool = False
    ) -> int | float:
        operator = 1
    ...
    @staticmethod
    def _get_final_stock_dict(
        old_items: dict[int, float], new_items: dict[int, float], transfer_type: str
    ) -> dict[int, float]:
        final_stock_dict = {}
        ...
        for commodity_id in all_ids:
            ...
            amount = TransactionsService._get_amount(
                transfer_type, new_amount - old_amount
            )
            if amount != 0:
