...
class PeriodWorkbook:
    ...
    def create_workbook(export_settings: dict[str, Path | str | float | bool], export_texts: dict[str, dict[str, str]],
                        in_data: list[ExportItemIn], out_data: list[ExportItemOut]) -> tuple[Workbook, float]:
        workbook = Workbook()
        ...
        _, last_balance = PeriodSheet.create_sheet(sheet, export_settings, period_texts, in_data, out_data)
