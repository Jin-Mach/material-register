...
class PeriodSheet:
    ...
    def create_sheet(
        ...
        row = PeriodSheet._create_header(
            sheet, row, PeriodSheet.LAST_COLUMN, export_settings, export_texts
        )
        row, buyback_cell, total_value = PeriodSheet._create_financial_section(
            sheet, row, PeriodSheet.LAST_COLUMN, export_settings, export_texts
        )
        freeze_row = row
        data_section_row = row
        in_section_row, buyback_value = PeriodSheet._create_data_in_section(
            sheet,
            data_section_row,
            PeriodSheet.LAST_COLUMN,
            export_texts,
            period_in_data,
        )
        total_value = PeriodSheet._update_buyback_value(
            buyback_cell, buyback_value, total_value
        )
        out_section_row = PeriodSheet._create_data_out_section(
            sheet,
            data_section_row,
            PeriodSheet.LAST_COLUMN,
            export_texts,
        ...
        row = PeriodSheet._create_data_spacer(
            sheet, in_section_row, out_section_row, PeriodSheet.LAST_COLUMN
        )
        PeriodSheet._auto_size_columns(sheet)
        page_text = export_texts.get("pageText", PeriodSheet.ERROR_TEXT)
        PeriodSheet._setup_sheet(
            sheet, row, PeriodSheet.LAST_COLUMN, freeze_row, page_text
        )
    ...
    def _create_header(
        ...
        cell = sheet.cell(
            row=row,
            column=1,
            value=export_texts.get("titleText", PeriodSheet.ERROR_TEXT),
        )
        sheet.merge_cells(
        ...
        period_value = PeriodSheet._get_period_range(
        ...
        sheet.merge_cells(
            start_row=row,
            start_column=branch_value_column,
            end_row=row,
    ...
    def _create_financial_section(
        ...
        cell = sheet.cell(
            row=row,
            column=1,
            value=export_texts.get("notesText", PeriodSheet.ERROR_TEXT),
        )
        sheet.merge_cells(
        ...
        sheet.merge_cells(
            start_row=row,
            start_column=1,
            end_row=row + PeriodSheet.NOTES_ROWS,
    ...
    def _create_data_in_section(
        ...
        last_column: int,
        export_texts: dict[str, str],
        in_data: dict[str, list[PeriodItemIn]],
    ) -> tuple[int, float]:
        start_row = row
        first_column = 1
        middle_column = last_column // 2
    ...
    def _create_data_out_section(
        ...
        last_column: int,
        export_texts: dict[str, str],
        data_out: dict[str, list[PeriodItemOut]],
    ) -> int:
        start_row = row
        middle_column = last_column // 2
        ...
        cell = sheet.cell(
            row=row,
            column=first_column,
            value=export_texts.get("exportText", PeriodSheet.ERROR_TEXT),
        )
        sheet.merge_cells(
            start_row=row,
            start_column=first_column,
        ...
        for category, category_data in data_out.items():
            ...
            sheet.row_dimensions[row].height = PeriodSheet.DEFAULT_ROW_HEIGHT
            row += 1
            row = PeriodSheet._create_category_out_section(
    ...
    @staticmethod
    def _create_category_in_section(
        sheet: Worksheet,
        row: int,
        last_column: int,
        export_texts: dict[str, str],
        in_data: list[PeriodItemIn],
        ...
        for item in in_data:
            ...
            cell = sheet.cell(
                row=row, column=commodity_column, value=item.commodity_name
            )
            PeriodSheet._cell_alignment(cell)
            PeriodSheet._cell_font(cell, bold=True)
            ...
            cell = sheet.cell(
                row=row, column=quantity_column, value=item.total_quantity
            )
    ...
    def _create_category_out_section(
        sheet: Worksheet, row: int, last_column: int, out_data: list[PeriodItemOut]
    ) -> int:
        middle_column = last_column // 2
        first_column = middle_column + 1
        commodity_column = first_column
        ...
        for index, item in enumerate(out_data):
            ...
            sheet.merge_cells(
                start_row=row,
                start_column=commodity_column,
                end_row=row,
                end_column=commodity_column + 1,
            )
            PeriodSheet._cell_alignment(cell)
            PeriodSheet._cell_font(cell, bold=True)
            cell = sheet.cell(
    ...
    @staticmethod
    def _create_data_spacer(
        sheet: Worksheet, in_row: int, out_row: int, last_column: int
    ) -> int:
        middle_column = last_column // 2
