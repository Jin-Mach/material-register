...
from material_register.ui.helpers.formatting_utils import format_date_range_to_locale
...
class SummarySheet:
    ...
    def _create_data_out_section(
        ...
        last_column: int,
        export_texts: dict[str, str],
        data_out: dict[str, list[SummaryItemDataOut]],
    ) -> int:
        start_row = row
        middle_column = last_column // 2
    ...
    def _create_category_out_section(
        sheet: Worksheet, row: int, last_column: int, out_data: list[SummaryItemDataOut]
    ) -> int:
        middle_column = last_column // 2
        first_column = middle_column + 1
        commodity_column = first_column
