...
def test_load_settings_valid(tmp_path: Path) -> None:
    ...
    _create_settings_file(tmp_path, data)
    SettingsProvider.provider_init(tmp_path)
    assert SettingsProvider.SETTINGS["export"]["default"]["branchNameLineEdit"] == "Some branch"
    assert SettingsProvider.SETTINGS["export"]["default"]["saveLastBalanceCheckbox"] is True
    assert SettingsProvider.SETTINGS["export"]["default"]["openingBalanceSpinbox"] == 1000.0
